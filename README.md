...
- **術科測驗模擬器操作擬真優化**：`public/exam-simulator-mr.html` 手動模式新增搖桿輸入可視化、姿態 HUD、風向推力指示與降落安全提示，讓使用者更清楚理解油門、偏航、前後/橫移、ATTI/POS、風場漂移與降落速度之間的關係。同步更新 SEO metadata、FAQ / noscript 術科說明與 sitemap 日期。

...
- 術科測驗模擬器手機版 UI 全面改善（`public/exam-simulator-mr.html`）：
  - 預設以**示範模式**開啟，自動播放術科流程、播放列立即可見
  - 左上角 📋 按鈕：隨時展開/收合術科流程面板；示範模式自動展開，手動模式自動收合
  - 右上角 📊 按鈕：隨時展開/收合 HUD（飛行狀態、視角高度、鍵盤提示），兩種模式預設隱藏
  - 示範流程面板即時高亮當前步驟（修復原本未跟隨播放進度的問題）
  - 移除畫面中央懸停計時器（P1 XX s/5 s），改善視覺整潔度
  - 播放列（上一步 / 暫停 / 下一步）與視角切換列間距加大，不再過度靠近
  - 修復觸控搖桿時畫面反白問題（補上 `user-select:none`）
  - 視角切換按鈕縮小並支援自動換行，防止手機上超出螢幕
  - 手動模式搖桿靈敏度大幅降低（moveSpd ×0.45、yawSpd ×0.5、throttle ×0.48），提升精準操控感

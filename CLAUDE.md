...
- `<link rel="icon" href="%BASE_URL%favicon.svg" type="image/svg+xml">` and `<link rel="apple-touch-icon">` — favicon for browser tab / iOS home screen; both use `%BASE_URL%` so Vite resolves correctly under the deploy sub-path
